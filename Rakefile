...
namespace :build do
  desc "Build ukti_template-#{UktiTemplate::VERSION}.gem into the pkg directory"
  task :gem => :compile do
    puts "Building pkg/ukti_template-#{UktiTemplate::VERSION}.gem"
    require 'packager/gem_packager'
    Packager::GemPackager.build
  end

  desc "Build ukti_template-#{UktiTemplate::VERSION}.tgz into the pkg directory"
  task :tar => :compile do
    puts "Building pkg/ukti_template-#{UktiTemplate::VERSION}.tgz"
    require 'packager/tar_packager'
    Packager::TarPackager.build
  end

  desc "Build play_ukti_template-#{UktiTemplate::VERSION}.tgz into the pkg directory"
  task :play => :compile do
    puts "Building pkg/play_ukti_template-#{UktiTemplate::VERSION}.tgz"
    require 'packager/play_packager'
    Packager::PlayPackager.build
  end

  desc "Build mustache_ukti_template-#{UktiTemplate::VERSION} into the pkg directory"
  task :mustache => :compile do
    puts "Building pkg/mustache_ukti_template-#{UktiTemplate::VERSION}"
    require 'packager/mustache_packager'
    Packager::MustachePackager.build
  end

  desc "Build liquid_ukti_template-#{UktiTemplate::VERSION} into the pkg directory"
  task :liquid => :compile do
    puts "Building pkg/liquid_ukti_template-#{UktiTemplate::VERSION}"
    require 'packager/liquid_packager'
    Packager::LiquidPackager.build
  end

  desc "Build mustache_inheritance_ukti_template-#{UktiTemplate::VERSION} into the pkg directory"
  task :mustache_inheritance => :compile do
    puts "Building pkg/mustache_inheritance_ukti_template-#{UktiTemplate::VERSION}"
    require 'packager/mustache_inheritance_packager'
    Packager::MustacheInheritancePackager.build
  end

  desc "Build jinja_ukti_template-#{UktiTemplate::VERSION} into the pkg directory"
  task :jinja => :compile do
    puts "Building pkg/jinja_ukti_template-#{UktiTemplate::VERSION}"
    require 'packager/jinja_packager'
    Packager::JinjaPackager.build
  end

  desc "Build django_ukti_template-#{UktiTemplate::VERSION} into the pkg directory"
  task :django => :compile do
    puts "Building pkg/django_ukti_template-#{UktiTemplate::VERSION}"
    require 'packager/django_packager'
    Packager::DjangoPackager.build
  end

  desc "Build ejs_ukti_template-#{UktiTemplate::VERSION} into the pkg directory"
  task :ejs => :compile do
    puts "Building pkg/ejs_ukti_template-#{UktiTemplate::VERSION}"
    require 'packager/ejs_packager'
    Packager::EJSPackager.build
  end

  desc "Build ukti_template-#{UktiTemplate::VERSION}.jar into the pkg directory"
  task :webjar => :compile do
    puts "Building pkg/ukti_template-#{UktiTemplate::VERSION}.jar"
  ...
  task :and_release_if_updated => :build do
    ...
    if q.version_released?
      puts "Github release v#{UktiTemplate::VERSION} already released. Not pushing."
    else
      puts "Pushing Github release v#{UktiTemplate::VERSION}"
      q.publish

      require 'publisher/docs_publisher'
      q = Publisher::DocsPublisher.new
      puts "Pushing docs #{UktiTemplate::VERSION} to git repo"
      q.publish
    end
    ...
    if q.version_released?
      ...
      puts "Pushing Github release v#{UktiTemplate::VERSION} to ruby gems"
      q.publish
    end
    ...
    if q.version_released?
      puts "ukti_template_play #{UktiTemplate::VERSION} already released. Not pushing."
    else
      puts "Pushing ukti_template_play #{UktiTemplate::VERSION} to git repo"
      q.publish
    end
    ...
    if q.version_released?
      puts "ukti_template_mustache #{UktiTemplate::VERSION} already released. Not pushing."
    else
      puts "Pushing ukti_template_mustache #{UktiTemplate::VERSION} to git repo"
      q.publish
    end
    ...
    if q.version_released?
      puts "ukti_template_ejs #{UktiTemplate::VERSION} already released. Not pushing."
    else
      puts "Pushing ukti_template_ejs #{UktiTemplate::VERSION} to git repo"
      q.publish
    end
    ...
    if q.version_released?
      puts "ukti_template_jinja #{UktiTemplate::VERSION} already released. Not pushing."
    else
      puts "Pushing ukti_template_jinja #{UktiTemplate::VERSION} to git repo"
      q.publish
    end
  end
end
