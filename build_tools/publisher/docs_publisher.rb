...
module Publisher
  class DocsPublisher
    GIT_URL = "git@github.com:dxw/ukti_template"

    def initialize(version = UktiTemplate::VERSION)
      @version = version
      @repo_root = Pathname.new(File.expand_path('../../..', __FILE__))
      @pkg_dir = @repo_root.join('pkg')
      @mustache_dir = @pkg_dir.join("mustache_ukti_template-#{@version}")
    end

    def publish
      Dir.mktmpdir("ukti_template_docs") do |dir|
        ...
        Dir.chdir(dir) do
          ...
          mustache_template = File.read("#{@mustache_dir}/views/layouts/ukti_template.html")
          ...
          run "git add -A ."
          run "git commit -q -m 'Publishing ukti_template examples version #{@version}'"
