...
module Publisher
  class MustachePublisher
    GIT_URL = "git@github.com:dxw/ukti_template_mustache"

    def initialize(version = UktiTemplate::VERSION)
      @version = version
      @repo_root = Pathname.new(File.expand_path('../../..', __FILE__))
      @source_dir = @repo_root.join('pkg', "mustache_ukti_template-#{@version}")
    end

    def publish
      Dir.mktmpdir("ukti_template_mustache") do |dir|
        ...
        Dir.chdir(dir) do
          ...
          run "git add -A ."
          run "git commit -q -m 'Publishing UKTI EIG {{ mustache }} template version #{@version}'"
