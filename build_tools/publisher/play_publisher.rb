...
module Publisher
  class PlayPublisher
    GIT_URL = "git@github.com:dxw/ukti_template_play"

    def initialize(version = UktiTemplate::VERSION)
      @version = version
      @repo_root = Pathname.new(File.expand_path('../../..', __FILE__))
      @source_dir = @repo_root.join('pkg', "play_ukti_template-#{@version}")
    end

    def publish
      Dir.mktmpdir("ukti_template_play") do |dir|
        ...
        Dir.chdir(dir) do
          ...
          run "git add -A ."
          run "git commit -q -m 'deploying UKTI EIG Play templates #{@version}'"
