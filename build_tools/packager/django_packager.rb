...
module Packager
  class DjangoPackager < TarPackager
    def initialize
      super
      @base_name = "django_ukti_template-#{UktiTemplate::VERSION}"
    end
    ...
    def parse_folders
      package_dir = @target_dir.join('ukti_template')
      package_dir.mkpath
      File.rename @target_dir.join('assets'), package_dir.join('static')
      File.rename @target_dir.join('views'), package_dir.join('templates')
      File.rename package_dir.join('templates', 'layouts'), package_dir.join('templates', 'ukti_template')
    end
    ...
    def generate_package_files
      ...
      File.new(@target_dir.join("ukti_template", "__init__.py"), "w").close
    end
    ...
    def generated_file_name(file_path)
      if file_path.include? "ukti_template"
        "base.html"
      else
