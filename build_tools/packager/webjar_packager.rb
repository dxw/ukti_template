...
module Packager
  class WebJarPackager
    ...
    def initialize
      ...
      @base_name = "ukti_template-#{UktiTemplate::VERSION}"
    end

    def build
      Dir.mktmpdir("ukti_template") do |dir|
        @target_dir = Pathname.new(dir).join('META-INF')
        @internal_dir = Pathname.new(@target_dir).join("resources/webjars/ukti_template/#{UktiTemplate::VERSION}/")
        @internal_dir.mkpath
        prepare_contents
        create_jar
      end
    ...
    def prepare_contents
      ...
      File.open(@target_dir.join('VERSION'), 'w') {|f| f.write "#{UktiTemplate::VERSION}\n" }
    end
