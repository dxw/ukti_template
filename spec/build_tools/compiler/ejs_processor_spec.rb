    ...
    context "if not stylesheet" do
      context "if css file path passed in" do
        ...
        it "should return the correct path for a stylesheet" do
          expect(subject.asset_path(css_asset_file)).to eql("<%= uktiTemplateAssetPath %>stylesheets/#{css_asset_file}?#{UktiTemplate::VERSION}")
        end
      end
      context "if javascript file path passed in" do
        ...
        it "should return the correct path for a javascript file" do
          expect(subject.asset_path(js_asset_file)).to eql("<%= uktiTemplateAssetPath %>javascripts/#{js_asset_file}?#{UktiTemplate::VERSION}")
        end
      end
      context "if other file path passed in" do
        let(:other_asset_file) {"a/file.png"}
        it "should return the correct path for an image" do
          expect(subject.asset_path(other_asset_file)).to eql("<%= uktiTemplateAssetPath %>images/#{other_asset_file}?#{UktiTemplate::VERSION}")
        end
      end
    end
  end
