...
describe Packager::JinjaPackager do
  let(:generated_directory_path) {File.join(PROJECT_ROOT, "pkg/jinja_ukti_template-#{UktiTemplate::VERSION}")}
  ...
  context "functional" do
    describe "build" do
      ...
      it "should output the correct template" do
        ...
        expect(generated_template).to match(%r[href="{{ asset_path }}stylesheets/ukti-template\.css\?#{Regexp.escape(UktiTemplate::VERSION)}"])
      end
