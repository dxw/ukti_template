...
describe Packager::EJSPackager do
  let(:generated_directory_path) {File.join(PROJECT_ROOT, "pkg/ejs_ukti_template-#{UktiTemplate::VERSION}")}
  let(:generated_template_path) {File.join(generated_directory_path, "views/layouts/ukti_template.html")}
  ...
  context "functional" do
    describe "build" do
      ...
      it "should output the correct template" do
        ...
        expect(generated_template).to match(%r[href="<%= uktiTemplateAssetPath %>stylesheets/ukti-template\.css\?#{Regexp.escape(UktiTemplate::VERSION)}"])
