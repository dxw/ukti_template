...
describe Packager::DjangoPackager do
  let(:generated_directory_path) {File.join(PROJECT_ROOT, "pkg/django_ukti_template-#{UktiTemplate::VERSION}")}
  let(:generated_template_path) {File.join(generated_directory_path, "ukti_template/templates/ukti_template/base.html")}
  ...
  context "functional" do
    describe "build" do
      ...
      it "should output the correct template" do
        ...
        expect(generated_template).to match(%r[href="{% static 'stylesheets/ukti-template\.css' %}"])
      end
